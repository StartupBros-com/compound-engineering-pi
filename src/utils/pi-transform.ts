...
function rewriteSlashCommandExecutionForPi(body: string): string {
	...
	result = result.replace(
		/Call the \/([a-z][a-z0-9_-]*) command/gi,
		(_match, commandName: string) => {
	...
	result = result.replace(
		/Run `\/([a-z][a-z0-9_-]*)([^`]*)`/gi,
		(_match, commandName: string, rawArgs: string) => {
			...
			const prompt =
				args.length > 0 ? `/${commandName} ${args}` : `/${commandName}`;
			if (background) {
	...
	if (
		result !== body &&
		!result.includes("Slash commands are Pi prompt templates")
	) {
		result +=
