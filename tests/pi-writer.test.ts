...
describe("writePiBundle", () => {
	test("removes stale generated agent skills without touching prompt files", async () => {
		const tempRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "pi-cleanup-targets-"),
		);
		const outputRoot = path.join(tempRoot, ".pi");

		await fs.mkdir(path.join(outputRoot, "skills", "session-historian"), {
			recursive: true,
		});
		await fs.writeFile(
		...
		expect(
			await exists(path.join(outputRoot, "skills", "session-historian")),
		).toBe(false);
		expect(
			await exists(path.join(outputRoot, "prompts", "session-historian.md")),
		).toBe(true);
	});
	...
	test("writes prompts, skills, extensions, mcporter config, and AGENTS.md block", async () => {
		...
		// the `subagent` tool.
		expect(
			await exists(path.join(outputRoot, "agents", "repo-research-analyst.md")),
		).toBe(true);
		expect(
			await exists(
		...
		expect(
			await exists(
				path.join(outputRoot, "compound-engineering", "mcporter.json"),
			),
		).toBe(true);
		expect(
			await exists(
	...
	test("transforms Task calls in copied SKILL.md files", async () => {
		const tempRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "pi-skill-transform-"),
		);
		const outputRoot = path.join(tempRoot, ".pi");
		const sourceSkillDir = path.join(tempRoot, "source-skill");
		await fs.mkdir(path.join(sourceSkillDir, "references"), {
			recursive: true,
		});
		await fs.writeFile(
...
`,
		...
		expect(installedSkill).toContain(
			'Run subagent with agent="code-simplicity-reviewer".',
		);
		...
		expect(installedReference).toContain(
			"Slash commands are Pi prompt templates",
		);
	...
	test("removes previously managed Pi artifacts that disappear on reinstall", async () => {
		const tempRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "pi-managed-cleanup-"),
		);
		const outputRoot = path.join(tempRoot, ".pi");
		...
		await writePiBundle(outputRoot, {
			...
			generatedSkills: [],
			agents: [
				{ name: "old-agent", content: "---\nname: old-agent\n---\n\nBody" },
			],
			extensions: [
				{
					name: "compound-engineering-compat.ts",
					content: "export default function first() {}",
				},
			],
		});
		...
		await writePiBundle(outputRoot, {
			...
			generatedSkills: [],
			agents: [
				{ name: "new-agent", content: "---\nname: new-agent\n---\n\nBody" },
			],
			extensions: [],
		});

		expect(
			await exists(path.join(outputRoot, "prompts", "old-prompt.md")),
		).toBe(false);
		expect(
			await exists(path.join(outputRoot, "prompts", "new-prompt.md")),
		).toBe(true);
		expect(
			await exists(path.join(outputRoot, "skills", "skill-one", "SKILL.md")),
		).toBe(false);
		expect(await exists(path.join(outputRoot, "agents", "old-agent.md"))).toBe(
			false,
		);
		expect(await exists(path.join(outputRoot, "agents", "new-agent.md"))).toBe(
			true,
		);
		expect(
			await exists(
	...
	test("namespaces managed install manifests per plugin so installs do not collide", async () => {
		const tempRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "pi-multi-plugin-"),
		);
		const outputRoot = path.join(tempRoot, ".pi");
		...
		await writePiBundle(outputRoot, {
			...
			agents: [],
			extensions: [
				{ name: "ce-ext.ts", content: "export default function () {}" },
			],
		});
		...
		await writePiBundle(outputRoot, {
			...
			agents: [],
			extensions: [
				{ name: "tutor-ext.ts", content: "export default function () {}" },
			],
		});
		...
		expect(
			await exists(
				path.join(outputRoot, "coding-tutor", "install-manifest.json"),
			),
		).toBe(true);
		...
		expect(await exists(path.join(outputRoot, "prompts", "ce-prompt.md"))).toBe(
			false,
		);
		expect(await exists(path.join(outputRoot, "skills", "ce-skill"))).toBe(
			false,
		);
		expect(await exists(path.join(outputRoot, "skills", "ce-gen-skill"))).toBe(
			false,
		);
		expect(await exists(path.join(outputRoot, "extensions", "ce-ext.ts"))).toBe(
			false,
		);
		expect(
			await exists(path.join(outputRoot, "prompts", "tutor-prompt.md")),
		).toBe(true);
		expect(await exists(path.join(outputRoot, "skills", "tutor-skill"))).toBe(
			true,
		);
		expect(
			await exists(path.join(outputRoot, "skills", "tutor-gen-skill")),
		).toBe(true);
		expect(
			await exists(path.join(outputRoot, "extensions", "tutor-ext.ts")),
		).toBe(true);
		expect(
			await exists(
				path.join(outputRoot, "coding-tutor", "install-manifest.json"),
	...
	test("moves stale compound-engineering mcporter.json to legacy backup when bundle has no mcporterConfig", async () => {
		const tempRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "pi-legacy-mcporter-"),
		);
		const outputRoot = path.join(tempRoot, ".pi");
		const staleConfigPath = path.join(
			outputRoot,
			"compound-engineering",
			"mcporter.json",
		);
		...
		expect(backedUp.mcpServers.stale?.baseUrl).toBe(
	...
	test("moves legacy flat Pi CE artifacts to a namespaced backup", async () => {
		const tempRoot = await fs.mkdtemp(
			path.join(os.tmpdir(), "pi-legacy-artifacts-"),
		);
		const outputRoot = path.join(tempRoot, ".pi");

		await fs.mkdir(path.join(outputRoot, "skills", "reproduce-bug"), {
			recursive: true,
		});
		await fs.writeFile(
		...
		await fs.mkdir(
			path.join(outputRoot, "skills", "bug-reproduction-validator"),
			{ recursive: true },
		);
		await fs.writeFile(
			...
			skillContent(
				...
				BUG_REPRODUCTION_VALIDATOR_DESCRIPTION,
			),
		);
		await fs.mkdir(path.join(outputRoot, "prompts"), { recursive: true });
		await fs.writeFile(
		...
		const plugin = await loadClaudePlugin(
			path.join(import.meta.dir, "..", "plugins", "compound-engineering"),
		);
		...
		expect(await exists(path.join(outputRoot, "skills", "reproduce-bug"))).toBe(
			false,
		);
		expect(
			await exists(
		...
		expect(
			await exists(path.join(outputRoot, "prompts", "reproduce-bug.md")),
		).toBe(false);
		expect(
			await exists(path.join(outputRoot, "prompts", "report-bug.md")),
		).toBe(false);
		expect(
			await exists(path.join(outputRoot, "skills", "ce-plan", "SKILL.md")),
		).toBe(true);
		// Compound Engineering no longer ships standalone agents; specialist
		// prompts live inside the consuming skill directories.
		expect(
			await exists(
				path.join(outputRoot, "agents", "ce-repo-research-analyst.md"),
			),
		).toBe(false);
		expect(
			await exists(
				path.join(outputRoot, "compound-engineering", "legacy-backup"),
			),
		).toBe(true);
	});
});
