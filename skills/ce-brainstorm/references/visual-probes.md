...
Use the platform's blocking question tool for the opt-in when available (`ask_user_question`, `request_user_input`, `ask_user`, or equivalent). Use a plain chat question only when no interactive question tool exists or the tool errors. The opt-in should have two clear options:
