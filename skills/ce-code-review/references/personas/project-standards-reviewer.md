...
- **Cross-platform portability violations** -- platform-specific tool names used without equivalents (e.g., `the platform's task-tracking primitive` instead of `the platform's task-tracking primitive`/`the platform's task-tracking primitive`/`the platform's task-tracking primitive`). Slash references in pass-through SKILL.md files that won't be remapped. Assumptions about tool availability that break on other platforms.
